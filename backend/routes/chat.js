...
그림 해석 규칙 (반드시 지켜주세요):
- 그림과 글을 함께 보며 글의 의미를 파악하세요. 그림에 없는 요소가 글에 등장하면 이름이나 배경 설명일 수 있어요
- 그림에서 실제로 보이는 것만 이야기하고, 보이지 않는 장면을 상상해서 추가하지 마세요
- 확실하지 않은 것은 할머니께 여쭤보세요
  ...
  try {
    ...
    if (validImage) {
      const caption = diaryText
        ? `할머니의 그림일기예요. 할머니가 직접 쓰신 내용: "${diaryText.slice(0, 200)}". 이 텍스트가 그림의 정답입니다. 텍스트에 있는 단어는 고유명사나 이름일 수 있으니 글자 그대로 받아들이세요. 그림에 실제로 보이는 것만 이야기하고, 보이지 않는 장면을 상상해서 추가하지 마세요.`
        : '할머니의 그림일기예요. 그림에 실제로 보이는 것만 이야기해주세요.';
      messages.push({
        role: 'user',
        content: [
